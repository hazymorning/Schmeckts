...
export async function read(name){
  if (!FS) {
    ...
    catch (e) { console.warn(`${KEYS[name]} is corrupted`, e); return null; }
  }
  let path = name + '.json', text = await readText(path);
  if (text == null) { path += '.tmp'; text = await readText(path); } // a crash between deleting and renaming
  if (text == null) return null;
  try { return JSON.parse(text); }
  catch (e) { // corrupted: set it aside instead of overwriting
...
async function writeNow(name, text){
  ...
  catch (e) { // if rename will not replace the target, delete first. If the app crashes in between, read() takes the .tmp
