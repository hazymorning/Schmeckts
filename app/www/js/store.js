...
function tidyState(s) {
  ...
  return {
    ...
    epoch: typeof o.epoch === 'string' ? o.epoch : '',
    seq: num(o.seq),
    offset: num(o.offset),
    ms: num(o.ms),
    n: num(o.n),
    clocks: Object.fromEntries(
      COLLECTIONS.map(c => [c, o.clocks?.[c] && typeof o.clocks[c] === 'object' ? o.clocks[c] : {}]),
    ),
...
const validChange = x =>
  x &&
  typeof x.id === 'string' &&
  COLLECTIONS.includes(x.c) &&
  typeof x.r === 'string' &&
  typeof x.t === 'string' &&
  x.f &&
  typeof x.f === 'object';

export let db = defaults(),
  prefs = defaultPrefs(),
  queue = [];
...
function persist(...names) {
  ...
  for (const n of names) {
    ...
    if (n === 'sync')
      schedule(
        'sync',
        () => ({text: JSON.stringify(stateForDisk()), ctx: connected() ? null : queue.map(x => x.id)}),
        trimJournal,
      );
  }
}
...
function ensureClocks() {
  ...
  for (const c of COLLECTIONS)
    for (const rec of db[c]) {
      ...
      const had = state.clocks[c][rec.id],
        clocks = had || {};
      for (const k of Object.keys(fieldsOf(c, rec)))
        if (!clocks[k]) {
          clocks[k] = ZERO();
          n++;
        }
  ...
  for (const c of COLLECTIONS) {
    ...
    for (const rec of db[c]) {
      ...
      if (Object.keys(ch).length) {
        out.push(change(c, rec.id, ch));
        markStale(c, rec, p && mealOf(p));
      }
    }
    for (const id of prev.keys())
      if (!cur.has(id)) {
        out.push(change(c, id, {_del: 'true'}));
        markStale(c, mealOf(prev.get(id)));
      }
    snap[c] = cur;
  }
  return out;
...
export function restamp(id) {
  ...
  rebase();
  const t = stamp(),
    clocks = state.clocks[x.c]?.[x.r] || {};
  for (const k of Object.keys(x.f)) if (clocks[k] === x.t) clocks[k] = t;
  x.t = t;
  persist('queue', 'sync');
}
export function setPosition(epoch, seq) {
  state.epoch = epoch;
  state.seq = seq;
  persist('sync');
}
