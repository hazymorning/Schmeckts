...
function clamp(c) {
  ...
  c.cy = within(c.cy, half, c.h - half);
  return c;
}
...
export function mountCrop(stage, img, c, slider) {
  const layout = () => {
    ...
    Object.assign(img.style, {
      width: c.w * k + 'px',
      height: c.h * k + 'px',
      transform: `translate(${-r.x * k}px, ${-r.y * k}px)`,
    });
    slider.value = c.z;
  };
  ...
  const gauge = () => {
    const l = [...pts.values()],
      n = l.length || 1;
    return {
      x: l.reduce((a, p) => a + p.x, 0) / n,
      y: l.reduce((a, p) => a + p.y, 0) / n,
      d: l.length > 1 ? Math.hypot(l[0].x - l[1].x, l[0].y - l[1].y) : 0,
    };
  };
