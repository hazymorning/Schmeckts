...
"""Generates everything that is not kept as source in the project. Only does what is missing.
  ...
  - from shared/recognize-prompt.txt the module app/www/js/prompt.js and the copy server/recognize-prompt.txt
    (server/recognize.go embeds that one with //go:embed; only the file in shared/ is maintained)
...
Usage: scripts/prepare.py                 normal
       ...
       scripts/prepare.py --fonts-only    fonts and prompt only (enough for tests in the browser)
       scripts/prepare.py --prompt-only   the prompt only (enough to build the server)
"""
...
def prompt():
    """The photo recognition prompt lives only in shared/recognize-prompt.txt: once as a module for the app and
    once as a copy in the Go package, because //go:embed cannot reach outside the package directory."""
    text = (ROOT / 'shared/recognize-prompt.txt').read_text(encoding='utf-8').strip()
    module = ('/* Generated by scripts/prepare.py from shared/recognize-prompt.txt; make changes there.\n'
              '   server/recognize.go embeds the same text (//go:embed recognize-prompt.txt). */\n'
    ...
    for path, data in ((APP / 'www/js/prompt.js', module), (ROOT / 'server/recognize-prompt.txt', text + '\n')):
        if not path.exists() or path.read_text(encoding='utf-8') != data:
            ...
            print('prompt generated:', path.relative_to(ROOT))
...
if __name__ == '__main__':
    prompt()
    if '--prompt-only' not in sys.argv:
        fonts()
    if not {'--fonts-only', '--prompt-only'} & set(sys.argv):
