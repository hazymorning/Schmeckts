...
- **Füttern:** Barcode scannen, Packung mit der eigenen Kamera fotografieren (ein Tipp, ohne Bestätigung; Marke und Sorte kommen aus der Erkennungskette) oder eine bekannte Sorte antippen. Gespeichert wird sofort, bewertet mit einem Tipp auf der Skala der Futterart, auf Wunsch mit Erinnerung. „Füttern beginnt mit“ blendet im Füttern-Sheet den Barcode- oder den Foto-Knopf aus; beide Wege bleiben über Kurzbefehle, Deep Links und das Foto nach einem unbekannten Barcode.
...
- **Auswertungs-Seite:** `report(db, prefs, jetzt, tage)` liefert alles für die Seite aus denselben Bewertungen, Gewichten und Schwellen; `reportModel()` in `derive.js` hält das Ergebnis wie das übrige Modell und rechnet erst beim Öffnen. Zeitraum 30 Tage (Standard), 90 Tage oder alles, dazu der Tier-Filter. Inhalt: Akzeptanz im Verlauf (gewichtete Wertung je Tag bei 30 Tagen, sonst je Woche, je Tier eine Linie), Verteilung der Bewertungen, die 6 häufigsten Marken, Konsistenz und Snack-Art (Gruppen ab 3 Bewertungen), Fütterungen je Wochentag und Person, zuletzt der Verlauf des Zeitraums. Abschnitte ohne genug Daten fehlen; unter 3 Bewertungen steht nur ein Satz.
...
**Karten:** Seite höchstens 600px, 18px Rand. Karte: `--surface`, Radius 26px, innen 18px oben und seitlich, 8px unten, 14px Abstand, ohne Rahmen und Schatten. Reihenfolge: Tiere-Leiste (ab zwei Tieren), Übersicht, „Wie war’s?“, Hinweis, „Letzte Woche“, „Verlauf“ (immer ganz sichtbar), „Einkaufen“, „Erkenntnisse“. Textknopf `.card-btn` („Alle anzeigen“, „Weitere anzeigen“, „Zur Auswertung“, nur wenn es mehr gibt, Zustand bis zum Neustart): volle Breite, Figtree 600, 14.5px, Akzenttext, Linie darüber.
...
- **Verlauf:** Zwei-Wochen-Kalender, darunter die letzten 5 Mahlzeiten nach Tagen; „Weitere anzeigen“ zeigt jeweils 5 mehr bis 20, danach führt „Ganzer Verlauf“ zur Auswertung beim Abschnitt Verlauf. Die Tageszeile zählt „2 Mahlzeiten, 1 Snack“ (Art Snack ist keine Mahlzeit, alles andere schon); sonst heißt es neutral „gefüttert“ (Rückblick, Meilenstein). Über dem Kalender 14px.
...
- **Tests:** `tests/*.test.js` prüfen reine Module in Node, die übrigen laufen in Chromium mit simulierten Plugins: `ui_test.py` (Abläufe), `storage_test.py`, `design_test.py` (auch pack/unpack), `sync_test.py` (mehrere Handys gegen den echten Server), `perf_test.py` (Neuzeichnen nach einer Bewertung unter 40 ms, Öffnen der Auswertung unter 150 ms, bei 5 Jahren Daten und CPU 4-fach gedrosselt). Einzeln: `python3 tests/ui_test.py modi`.
- **Austausch:** Es gilt Git. Der Austausch über die beiden Quellen-Dateien entfällt; sie liegen nicht mehr im Projekt. Eine Sitzung beginnt mit `git clone` oder `git pull` und einem eigenen Zweig, dann `setup-build-env.sh` und `test.sh`, am Ende Commit und Pull Request. Beschreibungen bleiben knapp: eine Zeile je Commit, im Pull Request nur, was sich geändert hat. Was `scripts/prepare.py` oder der Build wieder erzeugt (`app/node_modules/`, `app/android/`, `dist/`, die Schriften, `app/www/js/prompt.js` und `server/recognize-prompt.txt`), steht in `.gitignore` und gehört nicht ins Repository. `.github/workflows/tests.yml` fährt die Testsuite bei jedem Push und Pull Request auf `ubuntu-latest`, `.claude/hooks/session-start.sh` ergänzt einer Sitzung im Web die fehlenden Python-Pakete.
- **Release:** Version in `app/package.json` setzen, nach `main` zusammenführen, auf GitHub ein Release mit dem Tag `v<version>` veröffentlichen. `.github/workflows/release.yml` prüft Tag gegen Version, fährt die Tests, baut mit dem Geheimnis `SCHMECKTS_SIGNATUR` die signierte APK und hängt sie als `schmeckts-<version>.apk` ans Release; von Hand gestartet legt er sie als Artefakt ab.
