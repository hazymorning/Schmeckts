...
async def test_files(browser, url):
    ...
    check(
        writes[-2:] == [['writeFile', {'path': 'db.json.tmp', 'directory': 'DATA'}], ['rename', {'from': 'db.json.tmp', 'to': 'db.json'}]],
    ...
    check(
        await pg.evaluate(
            "JSON.parse(localStorage.getItem('__fs:db.json')).pets[0].name === 'Minka II' && localStorage.getItem('__fs:queue.json') === '[]'"
        ),
    ...
    check(
        len(aside) == 1 and await pg.evaluate(f"localStorage.getItem('{aside[0]}')") == '{kaputt',
        'a corrupted db.json is set aside, nothing is overwritten',
    )
    check(
